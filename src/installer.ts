...
export class Installer {
  static async installOc(versionInput: string): Promise<FindBinaryStatus> {
    ...
    const ocBinary = await Installer.downloadAndExtract(url, binaryVersion.type === 'version' ? binaryVersion.parsedVersion : undefined);
    return ocBinary;
  }
  ...
  static async downloadAndExtract(url: string, versionToCache: BinaryVersion | undefined): Promise<FindBinaryStatus> {
    ...
    if (!ocBinaryPath) {
      return {
        found: false,
        reason: `An error occurred while downloading and extracting oc binary from ${url}. File ${ocBinaryFilename} not found.`
      };
    }
  ...
  static async getLocalOcBinary(version: BinaryTypeVersion): Promise<string | undefined> {
    ...
    core.info(`Local oc version is ${localOcVersion}. Looking for ${version.parsedVersion.version}`);
    if (localOcVersion.parsedVersion.version === version.parsedVersion.version) {
      core.info(`oc ${localOcVersion} is already installed.`);
    }
  ...
  static async findOcFile(folder: string, file: string): Promise<string | undefined> {
    return new Promise<string | undefined>((resolve, reject) => {
      glob(`${folder}/**/${file}`, (err, res) => {
        if (err) {
          reject(new Error(`Unable to find ${file} inside the directory ${folder}: ${err}`));
        } else {
          resolve(res[0]);
        }
      });
